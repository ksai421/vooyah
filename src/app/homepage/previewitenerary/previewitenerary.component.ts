...
export class PreviewiteneraryComponent implements OnInit {
  ...
  // testLog(): void {
  //   this.AwsTranscationSyncService.log();
  // }
  toggle() {
    this.show = !this.show;
  ...
  async ngOnInit() {
    ...
    this.PreviewItenaryTotal = this.UserItinerary.userItineraryData();
    ...
    this.http
      ...
      .subscribe({
        next: async (data) => {
          ...
          let flightValidArray = this.sourceCode?.flights.map((x) => x.isValid);
          ...
          if (this.flights_valid === true && this.hotel_valid === true) {
            ...
            const id = uuidv1();
            let currentTransction: TransactionModel = {
              id: id,
              ...
              transactionId: id,
              transactionDetails: this.PreviewItenaryTotal,
              email: 'RamRoyal@gmail.com', // this.userDetails.email
              mobile: '+919059342087', //this.userDetails.mobileNo
