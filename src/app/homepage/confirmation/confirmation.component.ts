  ...
  async getTransactionsId() {
    try {
      ...
      for (let i = 0; i < this.transactions.length; i++) {
        let transactionStatus = this.transactions[i].transactionStatus;

        if (transactionStatus === 'completedSuccess') {
          this.showDetails();
          this.getIndividualDetails();
          this.isLoading = true;
          this.successPay = true;
        } else if (transactionStatus === 'completedFailed') {
          this.isLoading = true;
          this.bookingFail = true;
        } else if (transactionStatus === 'paymentSuccess') {
          this.getStatus();
          this.isLoading = true;
          this.paymentSuccess = true;
        } else if (transactionStatus === 'partialSuccess') {
          this.getStatus();
          this.isLoading = true;
  ...
  getIndividualDetails() {
    ...
    for (let i = 0; i < this.transactions.length; i++) {
      transactionsDetails.push(
        JSON.parse(this.transactions[i].transactionDetails)
      );
    }
    ...
    for (let i = 0; i < this.dayPlannerDetails.length; i++) {
      ...
      if (
        Object.keys(this.flightDetailsDb).length === 0 &&
        this.flightDetailsDb.constructor === Object
      ) {
        this.flightDetailsShow = true;
      }
      if (
        Object.keys(this.hotelDetailsDb).length === 0 &&
        this.hotelDetailsDb.constructor === Object
      ) {
